...
pub trait CompatibleDigestItem<Signature>: Sized {
	...
	/// If this item is an Jasmine seal, return the signature.
...
{
	fn aura_seal(signature: Signature) -> Self {
		DigestItem::Seal(JASMINE_ENGINE_ID, signature.encode())
	}

	fn as_aura_seal(&self) -> Option<Signature> {
		self.seal_try_to(&JASMINE_ENGINE_ID)
	}

	fn aura_pre_digest(slot: Slot) -> Self {
		DigestItem::PreRuntime(JASMINE_ENGINE_ID, slot.encode())
	}

	fn as_aura_pre_digest(&self) -> Option<Slot> {
		self.pre_runtime_try_to(&JASMINE_ENGINE_ID)
	}
}
