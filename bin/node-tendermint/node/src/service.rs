...
use node_tendermint_runtime::{self, opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		node_tendermint_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		node_tendermint_runtime::native_version()
	}
}
...
pub fn new_partial(
	...
	sc_service::PartialComponents<
		...
		(
			sc_finality_tendermint::TendermintBlockImport<FullBackend, Block, FullClient, FullSelectChain>,
...
pub fn new_full(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	println!("TDMT protocol name: {:?}", tendermint_protocol_name);

	config
		.network
		.extra_sets
		.push(sc_finality_tendermint::tendermint_peers_set_config(tendermint_protocol_name.clone()));
	// let warp_sync = Arc::new(sc_finality_tendermint::warp_proof::NetworkProvider::new(
	// 	backend.clone(),
	// 	tendermint_link.shared_authority_set().clone(),
	// 	Vec::default(),
	// ));
	...
	let enable_tendermint = !config.disable_tendermint;
