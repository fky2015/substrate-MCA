...
use sc_finality_pbft::SharedVoterState;
...
> {
	...
	let (pbft_block_import, pbft_link) = sc_finality_pbft::block_import(
		client.clone(),
	...
	let import_queue =
		sc_consensus_aura::import_queue::<AuraPair, _, _, _, _, _, _>(ImportQueueParams {
			block_import: pbft_block_import.clone(),
			justification_import: Some(Box::new(pbft_block_import.clone())),
			client: client.clone(),
	...
	Ok(sc_service::PartialComponents {
		...
		select_chain,
		transaction_pool,
		other: (pbft_block_import, pbft_link, telemetry),
	})
}
...
pub fn new_full(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	let sc_service::PartialComponents {
		...
		select_chain,
		transaction_pool,
		other: (block_import, pbft_link, mut telemetry),
	} = new_partial(&config)?;
	...
	println!("PBFT protocol name: {:?}", pbft_protocol_name);
	...
	let enable_pbft = !config.disable_pbft;
	...
	let pbft_config = sc_finality_pbft::Config {
		...
		keystore,
		telemetry: telemetry.as_ref().map(|x| x.handle()),
		protocol_name: pbft_protocol_name,
	};
	...
	if enable_pbft {
		...
		task_manager.spawn_essential_handle().spawn_blocking(
			"pbft-voter",
			None,
			sc_finality_pbft::run_pbft_voter(pbft_config)?,
		);
	}
