...
use crate::{
	...
	BlockNumberOps, PbftJustification, SharedAuthoritySet,
};
use sc_client_api::Backend as ClientBackend;
use sc_network::warp_request_handler::{EncodedProof, VerificationResult, WarpSyncProvider};
use sp_blockchain::{Backend as BlockchainBackend, HeaderBackend};
use sp_finality_pbft::{AuthorityList, SetId, PBFT_ENGINE_ID};
use sp_runtime::{
	generic::BlockId,
...
pub struct WarpSyncFragment<Block: BlockT> {
	...
	pub justification: PbftJustification<Block>,
}
...
impl<Block: BlockT> WarpSyncProof<Block> {
		...
		Backend: ClientBackend<Block>,
	{
		...
		for (_, last_block) in set_changes {
			...
			let justification = PbftJustification::<Block>::decode(&mut &justification[..])?;
