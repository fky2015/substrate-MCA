...
use crate::{
	...
	AuthoritySetChanges, ClientForPbft, CommandOrError, Error, NewAuthoritySet, VoterCommand,
};
...
impl<BE, Block: BlockT, Client, SC> JustificationImport<Block>
	for PbftBlockImport<BE, Block, Client, SC>
where
	...
	SC: SelectChain<Block>,
{
	...
	async fn import_justification(
		...
		PbftBlockImport::import_justification(self, hash, number, justification, false, false)
	}
}
...
impl<BE, Block: BlockT, Client, SC> PbftBlockImport<BE, Block, Client, SC>
where
	NumberFor<Block>: finality_grandpa::BlockNumberOps,
	BE: Backend<Block>,
	Client: ClientForPbft<Block, BE>,
	...
	TransactionFor<Client, Block>: 'static,
{
	...
	fn current_set_id(&self, id: &BlockId<Block>) -> Result<SetId, ConsensusError> {
		...
		if runtime_version.api_version(&<dyn PbftApi<Block>>::ID).map_or(false, |v| v < 3) {
			...
			for prefix in ["PbftFinality", "Pbft"] {
	...
	NumberFor<Block>: finality_grandpa::BlockNumberOps,
{
	...
	fn import_justification(
		...
		if justification.0 != PBFT_ENGINE_ID {
		...
		let justification = PbftJustification::decode_and_verify_finalizes(
			&justification.1,
			(hash, number),
		...
		match result {
			...
			Err(CommandOrError::Error(e)) => {
				return Err(match e {
					Error::Pbft(error) => ConsensusError::ClientImport(error.to_string()),
