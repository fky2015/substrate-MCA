...
use finality_grandpa::leader::{self, voter, VoterSet};
use futures::{channel::mpsc, future, stream, Stream, StreamExt, Sink};
use log::{debug, trace};
...
impl<Block: BlockT> Sink<(ViewNumber, Commit<Block>)> for CommitsOut<Block> {
	type Error = Error;
