...
use sp_finality_jasmine::{AuthorityList, SetId, ViewNumber};
use sp_runtime::traits::{Block as BlockT, NumberFor};

use crate::{
	authorities::{AuthoritySet, SharedAuthoritySet},
	environment::{SharedVoterSetState, VoterSetState},
	NewAuthoritySet, justification::JasmineJustification,
};

const VERSION_KEY: &[u8] = b"jasmine_schema_version";
const SET_STATE_KEY: &[u8] = b"jasmine_completed_round";
const CONCLUDED_ROUNDS: &[u8] = b"jasmine_concluded_rounds";
const AUTHORITY_SET_KEY: &[u8] = b"jasmine_voters";
const BEST_JUSTIFICATION: &[u8] = b"jasmine_best_justification";
...
pub(crate) fn load_decode<B: AuxStore, T: Decode>(
	...
	match backend.get_aux(key)? {
		...
		Some(t) => T::decode(&mut &t[..])
			.map_err(|e| ClientError::Backend(format!("JASMINE DB is corrupted: {}", e)))
			.map(Some),
	}
}
   ...
   //      },
		...
		Some(other) => {
			return Err(ClientError::Backend(format!(
				"Unsupported JASMINE DB version: {:?}",
				other
			)))
		},
        _ => {
			return Err(ClientError::Backend(format!(
				"Unsupported JASMINE DB version: None",
			)))
        }
	}

	// genesis.
	info!(target: "afp", "👴 Loading JASMINE authority set \
