...
pub use communication::jasmine_protocol_name::standard_name as protocol_standard_name;
pub use import::JasmineBlockImport;
pub use notification::{JasmineJustificationSender, JasmineJustificationStream};
...
pub struct LinkHalf<Block: BlockT, C, SC> {
	...
	justification_stream: JasmineJustificationStream<Block>,
	...
	fn get(&self) -> Result<AuthorityList, ClientError> {
		...
		self.executor()
			.call(
				...
				"JasmineApi_jasmine_authorities",
				&[],
...
pub fn block_import<BE, Block: BlockT, Client, SC>(
	...
	select_chain: SC,
	telemetry: Option<TelemetryHandle>,
) -> Result<(JasmineBlockImport<BE, Block, Client, SC>, LinkHalf<Block, Client, SC>), ClientError>
where
	SC: SelectChain<Block>,
	BE: Backend<Block> + 'static,
	Client: ClientForJasmine<Block, BE> + 'static,
{
	...
	Client: ClientForJasmine<Block, BE> + 'static,
{
	...
	let (voter_commands_tx, voter_commands_rx) = tracing_unbounded("mpsc_jasmine_voter_command");

	let (justification_sender, justification_stream) = JasmineJustificationStream::channel();
...
where
	BE: Backend<Block> + 'static,
	C: ClientForJasmine<Block, BE> + 'static,
	N: NetworkT<Block>,
...
where
	...
	C: ClientForJasmine<Block, B> + 'static,
	C::Api: JasmineApi<Block>,
...
pub struct JasmineParams<Block: BlockT, C, N, SC> {
	...
	/// to the configuration of the networking. See [`jasmine_peers_set_config`].
	pub network: N,
...
pub fn jasmine_peers_set_config(
...
pub fn run_jasmine_voter<Block: BlockT, BE: 'static, C, N, SC>(
	jasmine_params: JasmineParams<Block, C, N, SC>,
	...
	C::Api: JasmineApi<Block>,
{
	...
	// Make sure that `telemetry_task` doesn't accidentally finish and kill jasmine.
	let telemetry_task = telemetry_task.then(|_| future::pending::<()>());

	Ok(future::select(voter_work, telemetry_task).map(drop))
}

/// A trait that includes all the client functionalities jasmine requires.
/// Ideally this would be a trait alias, we're not there yet.
/// tracking issue <https://github.com/rust-lang/rust/issues/41517>
pub trait ClientForJasmine<Block, BE>:
...
impl<Block, BE, T> ClientForJasmine<Block, BE> for T
where
	Block: BlockT,
	BE: Backend<Block>,
...
/// Configuration for the JASMINE service.
#[derive(Clone)]
pub struct Config {
	...
	/// TelemetryHandle instance.
	pub telemetry: Option<TelemetryHandle>,
	/// Chain specific JASMINE protocol name. See [`crate::protocol_standard_name`].
...
/// Errors that can occur while voting in JASMINE.
#[derive(Debug, thiserror::Error)]
pub enum Error {
	/// An error within jasmine.
	#[error("jasmine error: {0}")]
	Jasmine(JasmineError),
...
impl From<JasmineError> for Error {
	fn from(e: JasmineError) -> Self {
		Error::Jasmine(e)
	}
}
