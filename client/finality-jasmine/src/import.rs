...
use sp_finality_jasmine::JasmineApi;
use sp_finality_jasmine::{ConsensusLog, ScheduledChange, SetId, JASMINE_ENGINE_ID};
use sp_runtime::{
...
use crate::{
	...
	justification::JasmineJustification,
	notification::JasmineJustificationSender,
	AuthoritySetChanges, ClientForJasmine, CommandOrError, Error, NewAuthoritySet, VoterCommand,
};

/// A block-import handler for JASMINE.
///
...
///
/// When using JASMINE, the block import worker should be using this block import
/// object.
pub struct JasmineBlockImport<Backend, Block: BlockT, Client, SC> {
	inner: Arc<Client>,
	select_chain: SC,
...
impl<Backend, Block: BlockT, Client, SC: Clone> Clone
	for JasmineBlockImport<Backend, Block, Client, SC>
{
	fn clone(&self) -> Self {
		JasmineBlockImport {
...
impl<BE, Block: BlockT, Client, SC> JustificationImport<Block>
	for JasmineBlockImport<BE, Block, Client, SC>
where
	NumberFor<Block>: finality_jasmine::BlockNumberOps,
	BE: Backend<Block>,
	Client: ClientForJasmine<Block, BE>,
	SC: SelectChain<Block>,
{
	...
	async fn import_justification(
		...
		JasmineBlockImport::import_justification(self, hash, number, justification, false, false)
	}
}
...
pub fn find_scheduled_change<B: BlockT>(
	...
	let id = OpaqueDigestItemId::Consensus(&JASMINE_ENGINE_ID);
...
impl<BE, Block: BlockT, Client, SC> JasmineBlockImport<BE, Block, Client, SC>
where
	NumberFor<Block>: finality_jasmine::BlockNumberOps,
	BE: Backend<Block>,
	Client: ClientForJasmine<Block, BE>,
	Client::Api: JasmineApi<Block>,
	for<'a> &'a Client:
	...
	TransactionFor<Client, Block>: 'static,
{
	...
	fn current_set_id(&self, id: &BlockId<Block>) -> Result<SetId, ConsensusError> {
		...
		if runtime_version.api_version(&<dyn JasmineApi<Block>>::ID).map_or(false, |v| v < 3) {
			...
			for prefix in ["JasmineFinality", "Jasmine"] {
...
#[async_trait::async_trait]
impl<BE, Block: BlockT, Client, SC> BlockImport<Block> for JasmineBlockImport<BE, Block, Client, SC>
where
	NumberFor<Block>: finality_jasmine::BlockNumberOps,
	BE: Backend<Block>,
	Client: ClientForJasmine<Block, BE>,
	...
	async fn import_block(
		...
		match applied_changes {
			AppliedChanges::Forced(new) => {
				...
				// triggered by this forced change. the new jasmine voter will start at the
		...
		let jasmine_justification =
			justifications.and_then(|just| just.into_justification(JASMINE_ENGINE_ID));

		match jasmine_justification {
			Some(justification) => {
				let import_res = self.import_justification(
					hash,
					number,
					(JASMINE_ENGINE_ID, justification),
					needs_justification,
					initial_sync,
				);
...
impl<Backend, Block: BlockT, Client, SC> JasmineBlockImport<Backend, Block, Client, SC> {
	pub(crate) fn new(
		inner: Arc<Client>,
		select_chain: SC,
		authority_set: SharedAuthoritySet<Block::Hash, NumberFor<Block>>,
	...
	NumberFor<Block>: finality_jasmine::BlockNumberOps,
{
	...
	fn import_justification(
		...
		if justification.0 != JASMINE_ENGINE_ID {
		...
		let justification = JasmineJustification::decode_and_verify_finalizes(
			&justification.1,
			(hash, number),
		...
		match result {
			...
			Err(CommandOrError::Error(e)) => {
				return Err(match e {
					Error::Jasmine(error) => ConsensusError::ClientImport(error.to_string()),
