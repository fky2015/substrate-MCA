...
pub struct WarpSyncFragment<Block: BlockT> {
	...
	pub justification: JasmineJustification<Block>,
}
		...
		Backend: ClientBackend<Block>,
	{
		...
		for (_, last_block) in set_changes {
			...
			let justification = blockchain
				...
				.and_then(|just| just.into_justification(JASMINE_ENGINE_ID))
				.expect(
			...
			let justification = JasmineJustification::<Block>::decode(&mut &justification[..])?;
