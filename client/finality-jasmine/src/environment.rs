...
/// The environment we run JASMINE in.
				...
				*voter_set_state = set_state;
				...
				if let Some(metrics) = self.metrics.as_ref() {
					if let VoterSetState::Live { completed_views, .. } = voter_set_state {
						...
						metrics.finality_jasmine_view.set(highest);
					}
				}
			}
			Ok(())
...
/// Prometheus metrics for JASMINE.
#[derive(Clone)]
pub(crate) struct Metrics {
	finality_jasmine_view: Gauge<U64>,
	finality_jasmine_prepares: Counter<U64>,
	finality_jasmine_commits: Counter<U64>,
}
...
impl Metrics {
	pub(crate) fn register(
		...
		Ok(Self {
			finality_jasmine_view: register(
				Gauge::new("substrate_finality_jasmine_round", "Highest completed JASMINE round.")?,
				registry,
			)?,
			finality_jasmine_prepares: register(
				Counter::new(
					"substrate_finality_jasmine_prepares_total",
					"Total number of JASMINE prevotes cast locally.",
				)?,
				registry,
			)?,
			finality_jasmine_commits: register(
				Counter::new(
					"substrate_finality_jasmine_commits_total",
...
pub(crate) enum JustificationOrCommit<Block: BlockT> {
	Justification(JasmineJustification<Block>),
...
impl<Block: BlockT> From<JasmineJustification<Block>> for JustificationOrCommit<Block> {
	fn from(justification: JasmineJustification<Block>) -> JustificationOrCommit<Block> {
...
pub(crate) fn finalize_block<BE, Block, Client>(
	...
	initial_sync: bool,
	justification_sender: Option<&JasmineJustificationSender<Block>>,
	...
	Client: ClientForJasmine<Block, BE>,
{
	...
	let update_res: Result<_, Error> = client.lock_import_and_run(|import_op| {
		...
		fn notify_justification<Block: BlockT>(
			justification_sender: Option<&JasmineJustificationSender<Block>>,
			justification: impl FnOnce() -> Result<JasmineJustification<Block>, Error>,
		) {
		...
		let persisted_justification = if justification_required {
			Some((JASMINE_ENGINE_ID, justification.encode()))
		} else {
			None
		};
